...
>
> Two sources were merged at consolidation: (1) the `## Recent coding-agent runs` block that lived inline in `docs/ai/project-state.md`, and (2) the `## Session Log` from the Obsidian vault main note (`lyricstamp.md`). Both are preserved verbatim below.
...
## From `docs/ai/project-state.md` — "Recently completed" (snapshot prose, preserved)

> These bullets lived under a `## Recently completed` heading in the old `project-state.md` snapshot. They were dropped from the lean snapshot at consolidation (not one of the target snapshot sections) and are preserved here verbatim.
...
## From the Obsidian vault main note (`lyricstamp.md`) — Session Log

<!-- Agents: after each work session, append a ### YYYY-MM-DD — title entry below -->

<!-- History below (2026-06-02 → 06-08) carried over from the pre-rebrand `ableset-lyrics-sync` note when that stale vault folder was consolidated on 2026-06-18. Repo was `ajhochy/ableset-lyrics-sync` before the rename to `ajhochy/lyricstamp`. -->

### 2026-06-02 — Electron wrapper + AI workflow scaffold
- **Issue 1 — Electron wrapper (`b049fa7`):** Wrapped the existing Vite/React + Node.js app in Electron via `electron-vite` so it ships as a standalone macOS `.app`.
- **AI workflow scaffold (`703169c`):** Added `AGENTS.md`, `docs/ai/*`, and `scripts/run_ai_workflow.py`.

### 2026-06-04 — Issues #26/27/28/30/32 fixed, packaged & signed macOS app, session persistence
- **#26/#27/#28 (`3b4dabe`):** Live 12 `.als` compat, ChordPro chords preserved, stamp preview relabeled.
- **Electron packaging:** chain of fixes to get the DMG to boot. Signing, static dir, template path, notarization hook all resolved.
- **#32 (`ad16e27`):** `.als` clip positions — `current_song_time` returns beats, not seconds. Removed bpm/60 conversion.
- **#30 (`3ed8d0c`):** pause-in-place — Space pauses + restores song time.
- **Session persistence (`960bb14`):** removed demo seed; `usePersistentState` persists to localStorage.

### 2026-06-05 — E2E harness + server-side session storage (PR #25)
- **E2E harness:** full-UI stamp-workflow spec + Playwright targets for dev, app, and packaged build.
- **Server-side session storage (`3ef1f19`, PR #25):** moved from browser IndexedDB to `server/src/session-store.ts`; shared across dev and packaged origins. Migrated 5 real sessions with PDFs.

### 2026-06-05 — live-stamp-write: Apply proofed stamps to Ableton (PR #32, merged)
- **Feature:** new "Apply to Ableton" button batch-writes proofed `+LYRICS` clips into the live Ableton Arrangement.
- **Vendored AbletonOSC fork** adding `duplicate_clip_to_arrangement` + `arrangement_writer_version` probe handlers.
- **Two Electron-only bugs caught in manual smoke:** clips were 1-beat (fixed span to next stamp); `window.prompt` unsupported in Electron (replaced with inline input).
- **Verified:** 114 unit · 35 e2e (build + packaged) · CI green. Manual Ableton smoke passed.

### 2026-06-05 — macOS notarization afterSign hook
- **Notarization (`574f3be`):** `scripts/notarize.cjs` — afterSign hook that notarizes + staples via `@electron/notarize`; no-ops locally without credentials.

### 2026-06-06 — Product rebranded "AbleSet Sync" → LyricStamp; licensed; v0.1.1/v0.1.2; public guide + in-app AbletonOSC installer
- **Leadsheet "Apply to Ableton" (`b2edcff`, #36):** proof-then-apply for PDF/leadsheet tab.
- **PolyForm Noncommercial 1.0.0 license** — source-available, free for noncommercial use.
- **Rebrand to LyricStamp (`6e14c32`, #40):** visible surfaces only; data paths kept as `ableset-*`.
- **v0.1.1 + v0.1.2 releases.**
- **Public guide + Cloudflare Worker (`0611899`, #42):** guide at `lyricstamp.vcrcapps.com`, public (no Cloudflare Access).
- **In-app AbletonOSC installer (#43):** one-click install from inside the app.

### 2026-06-08 — Shipped: guide live, v0.1.3 released (signed+notarized), manual smoke PASSED
- Guide live at `lyricstamp.vcrcapps.com`. PR #44 merged to main. v0.1.3 released (signed + notarized DMG). Manual smoke passed on real Ableton.

### 2026-06-11 — ChordPro chord-above-lyric preview (#27) + contracts for #30/#28
Branch `workflow/run-2026-06-11`. Two commits.

**#27 — ChordPro preview** [`e12ae3e`]
- Added a chord-above-lyric preview to the Lyrics setup panel. New **pure** module `client/src/chord-preview.ts` (`renderChordProHtml`) uses chordsheetjs `HtmlTableFormatter` to render the raw paste with chords positioned above lyrics, and directives (`{title}`, `{comment}`, section markers) rendered as labels.
- Wired into the `views.tsx` preview block — memoized, injected via `dangerouslySetInnerHTML` (safe because the formatter HTML-escapes text), styled with new `.chordpro-preview` rules.
- The server stamp path (`chordpro.ts`) is **untouched**, so the stamp log and `.als` clip names stay lyric-only. Closes #27.

**Contracts only for #30 / #28 (no code change)**
- #30 (pause-in-place) and #28 (stamp preview labels) were already implemented in `main` (`054b717`); this run adds executable acceptance contracts pinning that behavior.
- #30 c1/c2/c3 (true playhead retention) are Ableton-required manual smoke — `docs/testing/manual-smoke.md` Smoke 7; the OSC mapping is unit-pinned. Closes #30, #28.

**Verification**
- 145 unit, 10 contract, 52 Playwright e2e (incl. one new #27 preview-DOM test); typecheck / lint / build / `electron:build` all green.

**Retro** [`949d810`]
- Adherence postmortem for the #30/#27/#28 run (observability-only, strict run, no skill edits).

### 2026-06-12 — #30 pause-in-place fix: reorder seek-before-stop so continue_playing resumes from beat 0
- Manual Smoke 7 (the Ableton-required step from the #30 contract) caught a real bug the headless contracts had missed: single-stop *appeared* to reset the display to 1.1.1, but `continue_playing` resumed from the paused position rather than beat 0.
- **Root cause:** Ableton snapshots the playhead as the internal resume point for `continue_playing` when it processes `stop_playing`. The old order sent `set/current_song_time` **after** `stop_playing`, so the seek arrived after the snapshot — Ableton saved the pre-stop position and ignored the seek.
- **Fix (`3d2b9b9`):** in `server/src/osc-client.ts`, send `set/current_song_time 0` **first**, then `stop_playing`, so Ableton saves 0 as the resume point. Contract test c4 (`tests/contract/issue-30.spec.ts`) gains a positional-order assertion to catch this regression class headlessly. Smoke postmortem written (`2026-06-12-smoke-issues-30-27-28.json`).
